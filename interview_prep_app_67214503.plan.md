...
overview: Build InterviewTutor at D:/Project/InterviewTutor — senior full-stack interview prep with DSA/LLD/HLD, CS fundamentals, Java/.NET, AI doubt chat, and add-topic-from-chat.
todos:
  - id: scaffold-repo
    content: Scaffold git repo at D:/Project/InterviewTutor (React/Vite, .NET 8 API, Postgres, content/)
    status: pending
  - id: content-model
    content: Define Track/Module/Lesson/Problem schema and Markdown lesson format with Mermaid + code samples (Java/.NET)
    status: pending
  - id: lesson-ui
    content: Build tracks list, lesson reader (syntax-highlighted code), and progress tracking UI
    status: pending
  - id: seed-curriculum
    content: Seed MVP lessons across DSA, LLD, HLD, CS fundamentals, Java, and .NET with examples
    status: pending
  - id: ai-tutor
    content: Add doubt-clearing AI chat (full explanations, history) plus Explain / Quiz / Mock modes grounded in lesson context
    status: pending
  - id: add-topic-via-chat
    content: Detect "add this topic to lessons" in chat, generate lesson draft, user confirms, persist and show in track catalog
    status: pending
  - id: practice-mock
    content: Add small practice problem bank (Java + C#) and timed mock interview flow
    status: pending
isProject: false
---
...
## Assumed MVP
**Thin but useful MVP:**
1. Topic library covering all tracks above (seed depth varies; see curriculum)
2. Lesson pages with clear explanations + examples (Java and/or C# code blocks)
3. **Doubt-clearing AI chat** on every lesson — ask free-form questions, get full step-by-step explanations (plus Explain / Quiz / Mock modes)
4. Progress tracking (completed lessons, weak areas)
5. Small practice set (10–20 problems) with Java + C# solution walkthroughs — **no judge sandbox in v1**
6. Chat history saved per lesson so you can continue a doubt thread later
7. **Add-from-chat:** if you say “include this topic in lessons” (or similar), the app drafts a lesson, you confirm, and it appears in the catalog
...
| Track | MVP modules (examples) |
|-------|-------------------------|
| DSA | Arrays/Two pointers, Sliding window, Hashing, Trees/Graphs, Heaps, DP intro |
| LLD | OOP refresh, SOLID, common patterns, Parking lot / URL shortener design |
| HLD | URL shortener, Chat, News feed, Rate limiter, Cache design |
| Databases | Indexes, Transactions, Isolation, Query tuning basics, Redis use-cases |
| Networking | TCP vs UDP, HTTP lifecycle, DNS/TLS, LB/CDN |
| OS | Process vs thread, Sync primitives, Memory, Deadlocks |
| Java | Syntax/OOP, Collections, Streams, Concurrency, JVM GC, Spring Boot core |
| .NET | C# essentials, LINQ, async/await, GC, ASP.NET Core, EF Core |
| Senior FS | Auth, API design, caching, observability, behavioral |
...
1. **Detect intent** (LLM tool/function call or classifier) — extract topic title, suggested track (DB / Java / HLD / …), and optional notes from the conversation
2. **Check duplicates** — if a similar lesson already exists, offer that instead of creating another
3. **Generate a LessonDraft** via LLM using the same lesson template: concept → example (Java/C# as relevant) → interview Q&A → pitfalls → Mermaid if useful
4. **Show preview card in chat** — title, track, short summary, “Add to my lessons” / “Edit draft” / “Cancel”
5. **On confirm** — persist as a published `Lesson` in Postgres under the chosen track/module; appear immediately in `/tracks` and openable like seed lessons
6. **Doubt chat works on new lessons too** — same AI panel, now grounded in the newly added markdown
...
## Delivery phases
1. **Skeleton** — auth, grouped tracks list, lesson renderer (MD + Mermaid + code highlight), progress
2. **Curriculum seed** — modules above with at least 6–10 lessons per major track (Java/.NET and CS basics included)
3. **AI tutor** — doubt chat with lesson grounding; language preference (Java / C#)
4. **Add-from-chat** — intent detection, draft preview, confirm → personal lesson in catalog
5. **Practice + mock** — small dual-language problem bank + mock modes
6. **Polish** — search, bookmarks, spaced repetition, personalized study plan
